...
type MarketplaceConfig = {
  ...
  serviceTokenMasked: string | null;
  userAgent: string;
...
function resolveNextActionSummary(
  ...
  if (
    ...
    && item.importStatus === "ORDER_CREATED"
    && item.packageStatus !== "Picking"
    && item.packageStatus !== "Invoiced"
  ) {
    return {
      tone: "cyan" as const,
  ...
  if (capabilities.supportsPackageSplit && item.packageStatus === "Picking") {
    return {
      tone: "cyan" as const,
      text: labels.nextActionNotifyPicking,
    };
...
function resolvePackageListHint(
...
export function HepsiburadaIntegrationManager({
  labels,
  locale,
  canManage,
  initialConfigs,
  ...
  const [displayName, setDisplayName] = useState(configs[0]?.displayName ?? "Hepsiburada");
  const [sellerId, setSellerId] = useState(configs[0]?.sellerId ?? "");
  const [apiKey, setApiKey] = useState("");
  const [apiSecret, setApiSecret] = useState("");
  const [serviceToken, setServiceToken] = useState("");
  const [userAgent, setUserAgent] = useState(configs[0]?.userAgent ?? "");
  ...
  const [invoiceRowNumber, setInvoiceRowNumber] = useState("");
  ...
  async function refreshDashboard() {
    const response = await fetch("/api/admin/integrations/marketplaces/dashboard?channel=HEPSIBURADA");
  ...
  async function queueInvoiceSync() {
    if (!selectedPackage) {
      return;
    }
    ...
    try {
      const response = await fetch(`/api/admin/integrations/marketplaces/packages/${selectedPackage.id}/status-sync`, {
        ...
        body: JSON.stringify({
          ...
          ...(invoiceSerialNumber.trim() ? { invoiceSerialNumber: invoiceSerialNumber.trim() } : {}),
          ...(invoiceRowNumber.trim() ? { invoiceRowNumber: invoiceRowNumber.trim() } : {}),
        }),
      });
    ...
    <section className="grid gap-5">
      <div className="rounded-2xl border border-neutral-200 bg-white p-5">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Input value={serviceToken} onChange={(event) => setServiceToken(event.target.value)} placeholder={activeConfig?.serviceTokenMasked ?? labels.serviceToken} disabled={!canManage || busy} type="password" />
              ...
              <div className="flex items-center gap-2">
                ...
                {capabilities.supportsStatusPicking ? (
                  <Button
                    ...
                    disabled={!canManage || detailBusy || selectedPackage.importStatus !== "ORDER_CREATED" || selectedPackage.packageStatus === "Picking" || selectedPackage.packageStatus === "Invoiced"}
                    size="sm"
                  >
              ...
              <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  ...
                  <Button asChild type="button" variant="secondary" size="sm">
                    <Link href={`/${locale}/admin/integrations?channel=HEPSIBURADA&search=${encodeURIComponent(selectedPackage.id)}`}>
              ...
              {capabilities.supportsStatusInvoiced ? (
                <div className="grid gap-3 rounded-lg border border-neutral-200 bg-neutral-50 p-4 md:grid-cols-[1fr_160px_160px_auto] md:items-center">
                  <Input
                    ...
                    placeholder={labels.invoiceLink}
                    disabled={!canManage || detailBusy}
                  />
                  <Input
                    value={invoiceSerialNumber}
                    onChange={(event) => setInvoiceSerialNumber(event.target.value)}
                    placeholder={labels.invoiceSerialNumber}
                    disabled={!canManage || detailBusy}
                  />
                  <Input
                    value={invoiceRowNumber}
                    onChange={(event) => setInvoiceRowNumber(event.target.value)}
                    placeholder={labels.invoiceRowNumber}
                    disabled={!canManage || detailBusy}
                  />
                  <Button
                    type="button"
                    onClick={queueInvoiceSync}
                    disabled={!canManage || detailBusy || !canNotifyInvoiced || invoiceLink.trim().length === 0}
                    variant="secondary"
                  >
